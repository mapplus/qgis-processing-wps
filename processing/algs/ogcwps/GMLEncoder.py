...
class GMLEncoder:
    ...
    def encode_gml212(self, layer, identifier):
        # QGIS 2.2 버전에서 OGR GML 변환은 GML 2.1.2 버전임
        # 현재 선택된 피처가 없다면 모든 피처를 대상으로 분석
        ...
        if processing_count > float(WPSUtils.getProcessingMaxFeatures()):
            raise GeoAlgorithmExecutionException(u"원격에서 처리가능한 피처 수는 " + \
                 str(WPSUtils.getProcessingMaxFeatures()) + u" 개로 제한되어 있습니다.\r" + \
                 u"분석 대상 피처 수를 선택하거나 필터를 이용하여 처리될 피처의 수를 줄이십시오.\r" + \
                 u"또는 환경설정에서 [Maximum Features for processing vector layer] 값을 늘리십시오.\r" \
                 u"현재 레이어는 [" + layer.name() + u"] 입니다.")
        
        # write gml using ogr driver        
        gml_path = getTempFilenameInTempFolder(identifier + '.gml')
        error = QgsVectorFileWriter.writeAsVectorFormat(layer, gml_path, "utf-8", 
              layer.crs(), "GML", use_selected)
        if error != QgsVectorFileWriter.NoError:
            raise GeoAlgorithmExecutionException(u"GML로 변환할 수 없습니다.")
        else:
            return gml_path
    ...
    def encode_gml311(self, layer, identifier):
        ...
        writer.write('<wfs:FeatureCollection xmlns:feature="http://www.kopss.go.kr" xmlns:ogc="http://www.opengis.net/ogc"  xmlns:gml="http://www.opengis.net/gml"  xmlns:xlink="http://www.w3.org/1999/xlink"  xmlns:ows="http://www.opengis.net/ows"  xmlns:wfs="http://www.opengis.net/wfs">')
