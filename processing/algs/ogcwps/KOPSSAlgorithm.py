...
from processing.algs.ogcwps.WPSAlgorithm import WPSAlgorithm
from processing.algs.ogcwps.WPSProcess import *
from processing.algs.ogcwps.GMLEncoder import GMLEncoder

from processing.algs.ogcwps.wps import WebProcessingService, Process, Input, Output
from processing.algs.ogcwps.wps import WFSReferenceFeatureCollection, GMLFeatureCollection, BoundingBoxData
from processing.algs.ogcwps.wps import WKTGeometryData, FilterExpressionData, XMLData


class KOPSSAlgorithm(WPSAlgorithm):

    def __init__(self, group, process):
        # The WPS process that the user will use in the toolbox
        self.process = process

        WPSAlgorithm.__init__(self)

        # The branch of the toolbox under which the algorithm will appear
        self.group = group

        # The name that the user will see in the toolbox
        if u"ST_" in process.getIdentifier():
            self.name = process.getIdentifier()  # ST_Point
            if ":" in process.getIdentifier():
                self.name = process.getIdentifier().split(":")[1].strip()  # ST_Point
        else:
        ...
        for key, value in self.process.getDataInputs().iteritems():
            ...
            if str(value.getMinOccurs()) == "1":
                if param_value is None or len(param_value) == 0:
                   check_msg = value.getTitle().encode('utf8') +  u" 파라미터는 반드시 입력해야 합니다."
    ...
    def processAlgorithm(self, progress):
        ...
        if execution.isSucceded():
            ...
            QMessageBox.information(iface.mainWindow(), 'Information', 
                u'온라인 분석에 실패했습니다. 네트워크 상태 또는 변수 값을 확인하십시오.')
