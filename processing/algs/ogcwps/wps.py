...
# Modified: Minpa Lee mappus@gmail.com
# 
############################################
...
class WebProcessingService(object):
    ...
    def execute(self, identifier, inputs, outputs=None, request=None, response=None):
        """
        ...
        if request is None:
           requestElement = execution.buildRequest(identifier, inputs, outputs)
           request = etree.tostring( requestElement )   

           # replace special characterss in embed xml
           request = request.replace('&lt;', '<')
           request = request.replace('&gt;', '>')
           request = request.replace('&#65279;', '')

           #ProcessingLog.addToLog(ProcessingLog.LOG_INFO, request)
        if self.verbose==True:
               print request
...
class WPSExecution():
    ...
    def buildRequest(self, identifier, inputs=[], outputs=None):
        """
        ...
        outputs: optional identifier if process output is to be returned as a hyperlink reference
        """
        ...
        root = etree.Element('wps:Execute')
        ...
        identifierElement = etree.SubElement(root, 'ows:Identifier')
        identifierElement.text = identifier
        
        # <wps:DataInputs>
        dataInputsElement = etree.SubElement(root, 'wps:DataInputs')
        
        for input in inputs:
            key = input[0]
            val = input[1]
            
            inputElement = etree.SubElement(dataInputsElement, 'wps:Input')
            identifierElement = etree.SubElement(inputElement, 'ows:Identifier')
            ...
            if isinstance(val, str):
                dataElement = etree.SubElement(inputElement, 'wps:Data')
                literalDataElement = etree.SubElement(dataElement, 'wps:LiteralData')
        ...
        if outputs is not None:
            responseFormElement = etree.SubElement(root, 'wps:ResponseForm')
            responseDocumentElement = etree.SubElement(responseFormElement, 'wps:ResponseDocument', 
                                         attrib={'storeExecuteResponse':'false', 'status':'true'} )
            # decide vector or raster
            for output in outputs:
                key = output[0]
                mimeType = output[1]
                outputElement = etree.SubElement(responseDocumentElement, 'wps:Output', 
                                      attrib={'asReference':'true', 'mimeType':mimeType} )
                outputIdentifierElement = etree.SubElement(outputElement, 'ows:Identifier').text = key
    ...
    def getIdentifierOutput(self, identifier, filepath=None):
        """
...
class InputOutput(object):
    ...
    def _parseData(self, element):
        ...
        boundingBoxDataElement = element.find( nspath('BoundingBoxData', ns=getNamespace(element)) )
        if boundingBoxDataElement is not None:
        ...
        if literalDataElement is not None:
            ...
            for subElement in literalDataElement:
                ...
                if subElement.tag.endswith('DataType'):
                    reference = subElement.get( nspath("reference", ns=subns) )
                    if reference:
    ...
    def _parseBoundingBoxData(self, element, boundingBoxDataElementName):
        """
        ...
        #  <ows:Title>Extent of the output dataset</ows:Title>
        #  <ows:Abstract>The extent for the output gridcoverage</ows:Abstract>
        #  <BoundingBoxData>
        #    <Default>
        #      <CRS>EPSG:4326</CRS>
        #    </Default>
        ...
        #  </BoundingBoxData>
        #</Input>
        literalDataElement = element.find( boundingBoxDataElementName )
        if literalDataElement is not None:
            self.dataType = 'BoundingBoxData'                    
...
class Process(object):
    ...
    def findDataInput(self, identifier):
         for input in self.dataInputs:
            if input.identifier == identifier:
                return input

    def findProcessInput(self, identifier):
         for output in self.processOutputs:
            if output.identifier == identifier:
                return output
...
class BoundingBoxData():
    ...
    def __init__(self, extent, crs_code, dimensions=2):
        self.extent = extent
        self.crs_code = crs_code
        self.dimensions = str(dimensions)

    # "335537.522406,352130.977706,252377.158059,272096.58007"
    #<wps:Data>
    ...
    #</wps:Data>
    def getXml(self):
        dataElement = etree.Element('wps:Data')
        bboxDataElement = etree.SubElement(dataElement, 'wps:BoundingBoxData',
                                 attrib={"crs":self.crs_code, "dimensions":self.dimensions} )
        
        # xmin, xmax, ymin, ymax = qgis extent
        extent_list = self.extent.split(',')
        lowerCornerElement = etree.SubElement(bboxDataElement, 'ows:LowerCorner')
        lowerCornerElement.text = str(extent_list[0]) + ' ' + str(extent_list[2])
        upperCornerElement = etree.SubElement(bboxDataElement, 'ows:UpperCorner')
        upperCornerElement.text = str(extent_list[1]) + ' ' + str(extent_list[3])       

        #xmlstr = etree.tostring(dataElement, encoding='utf8', method='xml')
        #QMessageBox.information(iface.mainWindow(), 'BoundingBoxData', xmlstr)

        return dataElement
    
class WKTGeometryData():
    '''
    WKT Geometry
    '''
    ...
    def getXml(self):
        dataElement = etree.Element('wps:Data')
        complexDataElement = etree.SubElement(dataElement, 'wps:ComplexData',
                                 attrib={"mimeType":"application/wkt"} )
        complexDataElement.text = '<![CDATA[' + self.wkt + ']]>'

        #xmlstr = etree.tostring(dataElement, encoding='utf8', method='xml')
        #QMessageBox.information(iface.mainWindow(), 'getXml', xmlstr)

        return dataElement

class FilterExpressionData():
    '''
    ...
    #<wps:Data>
    #    <wps:ComplexData mimeType="text/plain; subtype=cql"><![CDATA[abc = 0]]></wps:ComplexData>
    #</wps:Data>
    def getXml(self):
        dataElement = etree.Element('wps:Data')
        complexDataElement = etree.SubElement(dataElement, 'wps:ComplexData',
                                 attrib={"mimeType":"text/plain; subtype=cql"} )
        complexDataElement.text = '<![CDATA[' + self.cql + ']]>'
...
class XMLData():
    ...
    #<wps:Data>
    #    <wps:ComplexData mimeType="text/xml"><![CDATA[ ..xml_contents.. ]]></wps:ComplexData>
    #</wps:Data>
    def getXml(self):
        dataElement = etree.Element('wps:Data')
        complexDataElement = etree.SubElement(dataElement, 'wps:ComplexData',
                                 attrib={"mimeType":"text/xml"} )
        complexDataElement.text = '<![CDATA[' + self.xml_contents + ']]>'
...
class WFSReferenceFeatureCollection(FeatureCollection):
    '''
    ...
    def __init__(self, reference_url):
        self.url = reference_url
        self.gml_version = 'text/xml; subtype=gml/3.1.1'
        if 'version=1.0.0' in reference_url.lower():
            self.gml_version = 'text/xml; subtype=gml/2.1.2'

    # <wps:Reference mimeType="text/xml; subtype=gml/3.1.1" xlink:href="http://61.79.176.34:8080/gxt/kopss/ows?service=WFS&version=1.1.0&request=GetFeature&typeName=kopss:rsa_trans_pp" method="GET"/>
    def getXml(self):
        dataElement = etree.Element('wps:Reference',
                             attrib = { "xlink:href" : self.url,
                                        "mimeType":self.gml_version,
                                        "method":"GET"} )
        #xmlstr = etree.tostring(dataElement, encoding='utf8', method='xml')
        #QMessageBox.information(iface.mainWindow(), 'getXml', xmlstr)
        return dataElement

class GMLFeatureCollection(FeatureCollection):
    '''
    FeatureCollection specified by a local data like shapefile...
    '''
    
    def __init__(self, gml_path, gml_version='text/xml; subtype=gml/3.1.1'):
    ...
    def getXml(self):
        with codecs.open(self.gml_path, 'r', "utf-8") as content_file:
            xml_contents = content_file.read()
            #QMessageBox.information(iface.mainWindow(), 'xml_contents', xml_contents)

            dataElement = etree.Element('wps:Data')
            complexDataElement = etree.SubElement(dataElement, 'wps:ComplexData',
                                     attrib={"mimeType":self.gml_version, 
                                             "encoding":"UTF-8"} )
            complexDataElement.text = '<![CDATA[' + xml_contents + ']]>'
